-- Update agreements_with_details view to show detailed payment breakdowns
...
LEFT JOIN (
    SELECT 
        ...
        SUM(tf.amount) as total_fines_amount,
        COUNT(CASE WHEN tf.payment_status = 'paid' THEN 1 END)::bigint as paid_fines_count,
        SUM(CASE WHEN tf.payment_status = 'paid' THEN tf.amount ELSE 0 END) as paid_fines_amount,
        COUNT(CASE WHEN tf.payment_status != 'paid' THEN 1 END)::bigint as pending_fines_count,
        SUM(CASE WHEN tf.payment_status != 'paid' THEN tf.amount ELSE 0 END) as pending_fines_amount
