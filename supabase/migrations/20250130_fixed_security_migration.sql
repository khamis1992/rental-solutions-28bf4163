-- Fixed Security Migration - Handles Constraints Properly
-- Addresses the constraint dependency issue
...
DO $$
BEGIN
    -- Check and handle leases_agreement_number_key constraint
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'leases' 
        AND constraint_name = 'leases_agreement_number_key'
        AND table_schema = 'public'
    ) THEN
        -- Check if there's a unique constraint we want to keep
        IF EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE table_name = 'leases' 
            AND constraint_name = 'leases_agreement_number_unique'
            AND table_schema = 'public'
        ) THEN
            -- Drop the duplicate constraint
            ALTER TABLE public.leases DROP CONSTRAINT IF EXISTS leases_agreement_number_key;
            RAISE NOTICE 'Dropped duplicate constraint leases_agreement_number_key';
        ELSE
            -- Rename the constraint to a better name
            ALTER TABLE public.leases RENAME CONSTRAINT leases_agreement_number_key TO leases_agreement_number_unique;
            RAISE NOTICE 'Renamed constraint to leases_agreement_number_unique';
        END IF;
    ELSE
        RAISE NOTICE 'leases_agreement_number_key constraint does not exist';
    END IF;

    -- Remove other duplicate indexes safely
