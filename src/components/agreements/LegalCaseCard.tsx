...
export default function LegalCaseCard({
  agreementId
}: LegalCaseCardProps) {
  const {
    toast
  ...
  const fetchLegalCases = async () => {
    try {
      ...
      const {
        data: agreement
      } = await supabase.from('leases').select('customer_id').eq('id', asLeaseIdColumn(agreementId)).single();
      if (agreement?.customer_id) {
        const {
          data,
          error
        } = await supabase.from('legal_cases').select(`
            *,
  ...
  const columns = [{
    ...
    cell: ({
      ...
      row: any;
          ...
          {row.original.status === 'resolved' ? <><FileCheck className="h-3 w-3 mr-1" /> RESOLVED</> : <><AlertTriangle className="h-3 w-3 mr-1" /> PENDING</>}
        </Badge>
  }, {
    ...
    cell: ({
      ...
      row: any;
    }) => `QAR ${row.original.amount_owed?.toLocaleString() || '0'}`
  }, {
