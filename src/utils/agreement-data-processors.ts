...
export const processAgreementData = (data: any): Agreement | null => {
  ...
  try {
    ...
    const processedAgreement: Agreement = {
      ...
      start_date: ensureDate(data.start_date)?.toISOString() || new Date().toISOString(),
      end_date: ensureDate(data.end_date)?.toISOString() || new Date().toISOString(),
      total_amount: data.total_amount || 0,
      created_at: ensureDate(data.created_at)?.toISOString(),
      updated_at: ensureDate(data.updated_at)?.toISOString(),
