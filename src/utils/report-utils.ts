...
export const generatePaymentHistoryPdf = (
  ...
  return generateStandardReport(title, dateRange, (doc, startY) => {
    ...
    const headers = ["Due Date", "Description", "Amount", "Payment Date", "Late Fee", "Total"];
    const columnWidths = [25, 45, 30, 30, 30, 30];
    ...
    payments.forEach((payment, index) => {
      ...
      row.forEach((cell, i) => {
        const textColor = i === 4 && payment.lateFee > 0 ? '#e74c3c' : '#2c3e50';
