...
export const generatePdfDocument = async (agreement: Agreement, language: 'en' | 'ar' = 'en'): Promise<boolean> => {
  try {
    ...
    docEn.setFontSize(10);
    docEn.setFont('helvetica', 'normal');

    // Add contract introduction
    docEn.text(`This vehicle rental agreement ("the Agreement") is made and executed as of the date ${formatDate(startDate)}.`, leftMargin, y);
    y += lineHeight * 2;

    // Parties section
    docEn.setFont('helvetica', 'bold');
    docEn.text('Between:', leftMargin, y);
    y += lineHeight * 2;
    ...
    const splitPartyOne = docEn.splitTextToSize(partyOneText, 170);
    docEn.text(splitPartyOne, leftMargin, y);
    y += splitPartyOne.length * lineHeight + lineHeight;

    docEn.text('And:', leftMargin, y);
    y += lineHeight * 2;
    ...
    const splitPartyTwo = docEn.splitTextToSize(partyTwoText, 170);
    docEn.text(splitPartyTwo, leftMargin, y);
    ...
    docEn.text(partiesText, leftMargin, y);
    y += lineHeight * 2;

    // Preamble
    docEn.setFont('helvetica', 'bold');
    docEn.text('Preamble', leftMargin, y);
    y += lineHeight * 2;

    docEn.setFont('helvetica', 'normal');
    docEn.text('Whereas Party One is a legally licensed car rental company and owns the vehicle described below:', leftMargin, y);
    y += lineHeight * 2;
    ...
    docEn.text(`Vehicle Type:`, leftMargin, y);
    y += lineHeight;
    docEn.text(`License Plate Number: ${vehiclePlate}`, leftMargin, y);
    y += lineHeight;
    docEn.text(`VIN: ${vehicleVin}`, leftMargin, y);
    y += lineHeight;
    docEn.text(`Vehicle Model: ${vehicleModel} - ${vehicleMake}`, leftMargin, y);
    y += lineHeight * 2;
    ...
    const splitPreamble = docEn.splitTextToSize(preambleText, 170);
    docEn.text(splitPreamble, leftMargin, y);
    y += splitPreamble.length * lineHeight + lineHeight;

    // Articles
    docEn.setFont('helvetica', 'bold');
    docEn.text('Article 1:', leftMargin, y);
    y += lineHeight;
    docEn.setFont('helvetica', 'normal');
    docEn.text('The preamble above forms an integral part of this Agreement and shall be interpreted within its terms and conditions.', leftMargin + 15, y);
    y += lineHeight * 2;

    // Article 2 - Vehicle Information
    docEn.setFont('helvetica', 'bold');
    docEn.text('Article 2 - Vehicle Information:', leftMargin, y);
    y += lineHeight;
    docEn.setFont('helvetica', 'normal');
    docEn.text('Party One hereby rents to Party Two the following vehicle:', leftMargin + 15, y);
    y += lineHeight * 2;

    docEn.text(`Vehicle Type:`, leftMargin + 15, y);
    y += lineHeight;
    docEn.text(`License Plate Number: ${vehiclePlate}`, leftMargin + 15, y);
    y += lineHeight;
    docEn.text(`VIN: ${vehicleVin}`, leftMargin + 15, y);
    y += lineHeight;
    docEn.text(`Model: ${vehicleModel} - ${vehicleMake}`, leftMargin + 15, y);
    y += lineHeight * 2;
    ...
    docEn.setFont('helvetica', 'bold');
    docEn.text('Article 3 - Rental Duration:', leftMargin, y);
    y += lineHeight;
    docEn.setFont('helvetica', 'normal');
    ...
    const splitDuration = docEn.splitTextToSize(durationText, 170);
    docEn.text(splitDuration, leftMargin + 15, y);
    y += splitDuration.length * lineHeight + lineHeight;

    // Article 4 - Rental Fee
    docEn.setFont('helvetica', 'bold');
    docEn.text('Article 4 - Rental Fee:', leftMargin, y);
    y += lineHeight;
    docEn.setFont('helvetica', 'normal');
    ...
    const splitFee = docEn.splitTextToSize(feeText, 170);
    docEn.text(splitFee, leftMargin + 15, y);
    ...
    docEn.setFont('helvetica', 'bold');
    docEn.text('Article 5 - Late Payment Penalties:', leftMargin, y);
    y += lineHeight;
    docEn.setFont('helvetica', 'normal');
    ...
    const splitPenalty = docEn.splitTextToSize(penaltyText, 170);
    docEn.text(splitPenalty, leftMargin + 15, y);
    y += splitPenalty.length * lineHeight + lineHeight;

    // Article 6 - Security Deposit
    docEn.setFont('helvetica', 'bold');
    docEn.text('Article 6 - Security Deposit:', leftMargin, y);
    y += lineHeight;
    docEn.setFont('helvetica', 'normal');
    ...
    const splitDeposit = docEn.splitTextToSize(depositText, 170);
    docEn.text(splitDeposit, leftMargin + 15, y);
    ...
    for (const article of remainingArticles) {
      ...
      docEn.setFont('helvetica', 'bold');
      docEn.text(article.title, leftMargin, y);
      y += lineHeight;
      ...
      for (const line of contentLines) {
        const splitContent = docEn.splitTextToSize(line, 160);
        docEn.text(splitContent, leftMargin + 15, y);
    ...
    docEn.setFont('helvetica', 'bold');
    docEn.text('In witness whereof, this Agreement is signed by the Parties in two copies, one for each Party.', leftMargin, y);
    y += lineHeight * 3;

    docEn.text('Party One:', leftMargin, y);
    docEn.text('Party Two:', leftMargin + 100, y);
    y += lineHeight;

    docEn.setFont('helvetica', 'normal');
    docEn.text('Represented by Mr. Khamees Hashem Al-Jaber', leftMargin, y);
    docEn.text(`Represented by Mr. ${customerName}`, leftMargin + 100, y);

    // Generate Arabic version
    const docAr = new jsPDF({
        orientation: 'portrait',
        unit: 'mm',
        format: 'a4',
        putOnlyUsedFonts: true
