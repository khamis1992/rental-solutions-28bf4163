...
export const useTrafficFines = () => {
  ...
  // Fetch all traffic fines with optimized query and improved error handling
  const { data: trafficFines, isLoading, error, refetch } = useQuery({
    queryKey: ['trafficFines'],
    queryFn: async () => {
      try {
        return await measurePerformance('fetchTrafficFines', async () => {
          ...
          // Optimize by querying only needed lease information
          ...
          if (finesWithLeaseIds.length > 0) {
            const leaseIds = finesWithLeaseIds.map(fine => fine.lease_id);
  ...
  // Memoized function to get fines by license plate
  const getFinesByLicensePlate = useCallback((licensePlate: string): TrafficFine[] => {
    if (!trafficFines) return [];
    return trafficFines.filter(fine => 
      fine.licensePlate?.toLowerCase() === licensePlate.toLowerCase()
    );
  }, [trafficFines]);
  
  // Memoized function to get fines by customer
  const getFinesByCustomer = useCallback((customerId: string): TrafficFine[] => {
    if (!trafficFines) return [];
    return trafficFines.filter(fine => fine.customerId === customerId);
  }, [trafficFines]);
  
  // Assign a traffic fine to a customer
  const assignToCustomer = useMutation({
    mutationFn: async ({ id }: TrafficFinePayload) => {
      return await measurePerformance('assignFineToCustomer', async () => {
        try {
  ...
  const payTrafficFine = useMutation({
    mutationFn: async ({ id }: TrafficFinePayload) => {
      return await measurePerformance('payTrafficFine', async () => {
  ...
  const disputeTrafficFine = useMutation({
    mutationFn: async ({ id }: TrafficFinePayload) => {
      return await measurePerformance('disputeTrafficFine', async () => {
        const { error } = await supabase
          .from('traffic_fines')
