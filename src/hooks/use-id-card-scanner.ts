...
export const useIdCardScanner = (options: ScanOptions = {}) => {
  ...
  const scanIdCard = useCallback(async (file: File): Promise<ScanResult> => {
    ...
    try {
      ...
      toast.loading('جاري مسح البطاقة الشخصية بـ Google Vision API...', {
      ...
      // Use Google Vision API for ID card scanning
      console.log('📝 Using Google Vision API for ID card scanning');
      
      // Extract text using Google Vision OCR service
      ...
      if (result.success && result.data) {
        ...
        toast.success(`تم المسح بنجاح بـ Google Vision API! دقة النتائج: ${result.confidence}%`);
      } else {
